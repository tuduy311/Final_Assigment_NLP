        ...
        {summaryResult && (
          <ResultSection title="Summary & Action Items" defaultOpen={true}>
            <SummaryPanel 
              ...
              userName={userName}
              audioId={workspaceData?.audio_id}
            />
          </ResultSection>
        )}
        ...
        {transcriptResult && (
          <ResultSection title="Transcript" defaultOpen={!summaryResult}>
            <TranscriptPanel
              ...
              editableTranscriptSegments={editableTranscriptSegments}
              mergedDisplayLines={mergedDisplayLines}
...
const ActionButton = ({ title, onClick, isLoading, isDone, disabled, activeStyle = false }) => {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`relative p-4 rounded-xl border-2 text-left transition-all ${disabled && !isDone
